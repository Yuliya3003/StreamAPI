...
/*
1. Осуществлять учет продовольственных и непродовольственных товаров, которыми торгует магазин
...
который включает все основные питательные элементы.
 ...
 Особенности бизнес-логики сервиса
 ...
 которую выбрал покупатель посреством настроен фильтрации(фильтрация по видам)

 Необходимо предусмотреть типобезопасность онлайн корзины, так как в момент "автоматической балансировки"
она должна пополняться продовольственными товарами из списка товаров U-Market СФУ.
...
С целью упрощения алгоритма балансировки, будем считать, что каждый товар содержит всего один питательный элемент: белки, жиры, углеводы
        ...
        while(true){
            ...
            if (scanner.hasNextInt()){
                ...
                if(no ==0){
                    ...
                    T thing = (T) market.getThingByIndex(clazz, no);
                    if (thing == null){
                        System.out.println("Некорректные номер товара.\nПожалуйста, повторите попытку ввода.");
                        continue;
                    }
